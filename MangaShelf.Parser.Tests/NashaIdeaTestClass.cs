...
public class NashaIdeaTestClass : BaseParserTestClass<NashaIdeaParser>
{
    ...
    {
        var result = await Parser.Parse("https://nashaidea.com/product/boiovi-shramy/");
    ...
    {
        var result = await Parser.Parse("https://nashaidea.com/product/stalevyj-alhimik-tom-8/");

        Assert.IsNotNull(result);
        using (new AssertionScope())
        {
            Assert.AreEqual(18, result.TotalVolumes);
            Assert.AreEqual(SeriesStatus.Completed, result.SeriesStatus);
        }
    }
    ...
    {
        var html = File.ReadAllText($"TestData/NashaIdea_PreorderDate_SholdBe_LastDayOfMonth.html");
        var result = await Parser.Parse("https://nashaidea.com/product/stalevyj-alhimik-tom-6/", html, CancellationToken.None);
